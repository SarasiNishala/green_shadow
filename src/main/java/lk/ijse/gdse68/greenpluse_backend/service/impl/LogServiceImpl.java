...
import lk.ijse.gdse68.greenpluse_backend.service.LogService;
...
public class LogServiceImpl implements LogService {
    ...
    public CropDetailsResponse getSelectedLog(int logCode) {
        if (logDAO.existsById(logCode)) {
            return (CropDetailsResponse) mapping.convertToCropDetailsDTO(logDAO.getReferenceById(logCode));
        } else {
            throw new NotFoundException("Log not found for code: " + logCode);
        }
    }
